        ...
        <nav className="flex-1 p-4 space-y-4 overflow-y-auto">
          {menuSections.map((section, sectionIdx) => (
            <div key={sectionIdx}>
              {/* Section Header */}
              {isSidebarOpen && (
                <h3 className="text-xs font-semibold text-indigo-300 px-4 mb-2 uppercase tracking-wider">
                  {section.title}
                </h3>
              )}
              {!isSidebarOpen && (
                <div className="border-t border-indigo-500 mb-2"></div>
              )}
        ...
        <header className="bg-white shadow-sm">
          <div className="px-6 py-4">
            <h2 className="text-2xl font-bold text-gray-800">
              {menuSections.flatMap(section => section.items).find(item => item.path === location.pathname)?.label || 'Dashboard'}
            </h2>
          </div>
        </header>
